...
public class Test02Controller {
	...
	public ModelAndView calc(HttpServletRequest request, HttpServletResponse response) {
		...
		String[] hobby = request.getParameterValuse("hobby");
	...
	*/
	...
	/*
	  @PostMapping("calc.do") // 파라미터값을 vo에 담는다.(vo 에 있는 변수는 파라미터이름과 같아야 한다.) //
	  스프링에서는 vo로 받는게 편하다. public ModelAndView calc(VO vo) { ModelAndView mv = new
	  ModelAndView("test02/result"); int su1 = Integer.parseInt(vo.getS1()); int
	  su2 = Integer.parseInt(vo.getS2());
	  int result = test02Service.getCalc(su1, su2, vo.getOp());
	  vo.setResult(String.valueOf(result)); // result값을 String 으로 변환
	  mv.addObject("vo", vo); // VO에 있는 파라미터값들이 넘어온다. return mv; }
	 */
